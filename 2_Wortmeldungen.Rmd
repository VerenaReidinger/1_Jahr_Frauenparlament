...
gemäss https://de.wikipedia.org/wiki/Frauen_in_der_Politik#Schweiz
```{r, results = "markup"}
lkp_Anteile %>% ggplot(aes(x = Legislatur, y = FrauenanteilParlament, group = "")) +
                geom_line() + 
                ggtitle("Frauenanteil im Nationalrat\ [%]") +
                ylim(0, 0.5)
...
res_anzahl <- df_NR_nP %>% group_by(Legislatur, GenderAsString) %>% 
                           ...
                           mutate(Anteil_Zeit_f = Dauer_f  / (Dauer_f  + Dauer_m)) %>% 
                           mutate(Anteil_Zeit_m = Dauer_m  / (Dauer_f  + Dauer_m)) %>% 
                           left_join(lkp_Anteile, by = "Legislatur") %>% 
                           left_join(mot_leg[, c("Legislatur", "Mot_Anteil_f")], by = "Legislatur") %>% 
